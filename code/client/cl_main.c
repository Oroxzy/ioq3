...
cvar_t	*cl_aimAssistEdge;
cvar_t	*cl_aimAssistLead;
cvar_t	*cl_aimAssistLearn;
cvar_t	*cl_aimAssistAttacker;
cvar_t	*cl_botOutline;
cvar_t	*cl_botOutlineStyle;
cvar_t	*cl_botOutlineColor;
cvar_t	*cl_botOutlineName;
cvar_t	*cl_botOutlineBars;
cvar_t	*cl_damagePlums;
...
void CL_Init( void ) {
	...
	Cvar_SetDescription( cl_botOutline, "Mark enemy bots through walls on a local server; 2 also shows what they had left the last time they were hit" );
	cl_botOutlineStyle = Cvar_Get( "cl_botOutlineStyle", "2", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_botOutlineStyle, 0, 3, qtrue );
	Cvar_SetDescription( cl_botOutlineStyle, "How a bot is marked: 0 the old wire box, 1 a filled silhouette of the model, 2 a contour line around it, 3 both" );
	cl_botOutlineColor = Cvar_Get( "cl_botOutlineColor", "255 0 220", CVAR_ARCHIVE );
	Cvar_SetDescription( cl_botOutlineColor, "The bot silhouette/contour colour as \"r g b\", each 0 to 255 - the default is magenta so a bot never blends into the item boxes" );
	cl_botOutlineName = Cvar_Get( "cl_botOutlineName", "0", CVAR_ARCHIVE );
	...
	cl_aimAssistSmooth = Cvar_Get( "cl_aimAssistSmooth", "0", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_aimAssistSmooth, 0, 300, qtrue );
	Cvar_SetDescription( cl_aimAssistSmooth, "Milliseconds the aim assist takes to follow the target between shots; 0 follows it at once" );
	cl_aimAssistExact = Cvar_Get( "cl_aimAssistExact", "1", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_aimAssistExact, 0, 2, qtrue );
	Cvar_SetDescription( cl_aimAssistExact, "On the command a shot fires on, put the aim exactly on the predicted point: 1 for single-shot weapons, 2 for all" );
	cl_aimAssistFreeze = Cvar_Get( "cl_aimAssistFreeze", "0", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_aimAssistFreeze, 0, 1, qtrue );
	Cvar_SetDescription( cl_aimAssistFreeze, "Drop the player's own mouse, turn-key and joystick view input while the aim assist is really steering at a target" );
	cl_aimAssistEdge = Cvar_Get( "cl_aimAssistEdge", "0", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_aimAssistEdge, 0, 1, qtrue );
	Cvar_SetDescription( cl_aimAssistEdge, "Apply the fall when the predicted run leaves a platform; 0 only detects it and logs edge 2" );
	cl_aimAssistLead = Cvar_Get( "cl_aimAssistLead", "1.5", CVAR_ARCHIVE );
	Cvar_CheckRange( cl_aimAssistLead, 0.1f, 5.0f, qfalse );
	...
	Cvar_SetDescription( cl_hitPitchFull, "Hit sound pitch when the target has cl_hitPitchStack or more health and armor left" );
